####READ ME####
#This script contains code to replicate the main results in the manuscript.
#The script is divided into sections for the different steps.
#Some notes and annotations are included in each section.

#R-version used: R 4.2.2. Has not been tested if it runs on other versions.
...
install.packages(c("igraph","MASS", "NetIndecies", "fluxweb", "NetworkExtinctions",
                   "network", "patchwork", "scales", "tidyverse", "fmsb"))
#load
library(igraph) #network calculations and visualization
library(MASS)
library(NetIndices)#calculation of information-theoretic network (AMI/ relative ascendency) indices
...
library(fmsb) # includes Pseudo-R^2 function
...
# fluxind(), custom function from https://rfrelat.github.io/BalticFoodWeb.html
...
####Weighted connectance calculation####
w.metrics<-list()
for (i in 1:1000){
  #i<-1
  a<-w.webs.1000[[i]]
...
g.con<-ggplot(df.con, aes(x=Year))+
  ...
  ylab("C")+theme(axis.title=element_text(size=18))+
  scale_y_continuous(labels = label_number(accuracy = 0.01),
                     breaks = scales::pretty_breaks(n = 5))+
  theme(plot.margin = unit(c(1.2,1,1,0.5), "cm"))
...
g.wcon<-ggplot(df.wcon, aes(x=Year))+
  ...
  xlab("Year")+ylab("lwC")+theme(axis.title=element_text(size=18))+
  theme(plot.margin = unit(c(1.2,1,1,0.5), "cm"))
...
g.uw.rel.asc<-ggplot(df, aes(x=Year))+
  ...
  xlab("Year")+ylab("A/C unweighted")+theme(axis.title=element_text(size=18))+
  theme(plot.margin = unit(c(1.2,1,1,0.5), "cm"))

# relative ascendecny weighted webs
df<-matrix(NA,34,6)
colnames(df)<-c("Year","Median","upCI95","lowCI95", "upCI50", "lowCI50")
df[,1]<-c(seq(1981,2014))
...
g.rel.asc<-ggplot(df, aes(x=Year))+
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5),
                     labels = label_number(accuracy = 0.01))+
  xlab("Year")+ylab("A/C weighted")+theme(axis.title=element_text(size=18))+
  theme(plot.margin = unit(c(1.2,1,1,0.5), "cm"))


#all in one plot

fig.2<-g.con+labs(tag = "(a)")+theme(plot.tag = element_text(hjust = 1, size = 22,face = "bold"))+
  ...
  g.wcon+labs(tag = "(b)")+theme(plot.tag = element_text(hjust = 0.5, size = 22,face = "bold"))+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.title.x = element_text(vjust = -3))+
  theme(axis.title.y = element_text(vjust = 5))+
  g.uw.rel.asc+labs(tag = "(c)")+theme(plot.tag = element_text(hjust = 0.5, size = 22,face = "bold"))+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.title.x = element_text(vjust = -3))+
  theme(axis.title.y = element_text(vjust = 5))+
  g.rel.asc+labs(tag = "(d)")+theme(plot.tag = element_text(hjust = 0.5, size = 22,face = "bold"))+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.text.y = element_text(angle = 0, vjust = 0.5, size = 18))+
  theme(axis.title.x = element_text(vjust = -3))+
  theme(axis.title.y = element_text(vjust = 5))+
...
jpeg(filename = "Figure 2.jpeg", width = 16, height = 10, units = "in", res=300)
fig.2
dev.off()
...
uw.R50<-matrix(NA, 34, 1000)
uw.R50.th10<-matrix(NA, 34, 1000)
uw.R50.th20<-matrix(NA, 34, 1000)
uw.R50.th30<-matrix(NA, 34, 1000)
uw.R50.th40<-matrix(NA, 34, 1000)
uw.R50.th50<-matrix(NA, 34, 1000)
uw.R50.th60<-matrix(NA, 34, 1000)
uw.R50.th70<-matrix(NA, 34, 1000)
uw.R50.th80<-matrix(NA, 34, 1000)
uw.R50.th90<-matrix(NA, 34, 1000)
...
df.allTH[,12]<-c("A                 Unweighted webs")
colnames(df.allTH)<-c("No threshold","90%","80%", "70%", "60%",
...
df.allTH<-matrix(NA,34,12)
df.allTH[,11]<-c(seq(1981,2014))
df.allTH[,12]<-c("B                 Unweighted webs")
colnames(df.allTH)<-c("No threshold","90%","80%", "70%", "60%",
                      "50%","40%", "30%", "20%", "10%", "Year", "webs")
for (i in 1:10){
  df.allTH[,i]<-apply(wR50.list[[i]],1, median)
}
as.data.frame(df.allTH)
d.w<-pivot_longer(as.data.frame(df.allTH), cols = 1:10)

d.uw$Year<-d.uw$Year<-as.numeric(as.character(d.uw$Year))
d.uw$value<-d.uw$value<-as.numeric(as.character(d.uw$value))
colnames(d.uw)[3]<-c("Thresholds")
d.uw$Thresholds<-factor(d.uw$Thresholds, levels = c("No threshold","90%","80%", "70%", "60%",
                                            "50%","40%", "30%", "20%", "10%"))
fig3a<-ggplot(d.uw, aes(x=Year, y=value, color = Thresholds))+
  geom_line(aes(group=Thresholds),linewidth=1.1)+
  ...
  coord_cartesian(clip = "off")+theme(plot.margin = margin(0,0,2,0, "cm"))

d.w$Year<-d.w$Year<-as.numeric(as.character(d.w$Year))
colnames(d.w)[3]<-c("Thresholds")
d.w$value<-d.w$value<-as.numeric(as.character(d.w$value))
d.w$Thresholds<-factor(d.w$Thresholds, levels = c("No threshold","90%","80%", "70%", "60%",
                                        "50%","40%", "30%", "20%", "10%"))

fig3b<-ggplot(d.w, aes(x=Year, y=value, color = Thresholds))+
  geom_line(aes(group=Thresholds),linewidth=1.1)+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, size = 25))+
  theme(axis.text.y = element_text(angle = 0,vjust = 0.5, size = 25))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  xlab("Year")+ylab("")+theme(axis.title=element_text(size=28), axis.title.x = element_text(angle = 0),
                                 axis.title.y.left = element_text(angle = 90))+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5))+
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))+
  scale_color_brewer(palette="Spectral")+
...
fig3comb<-fig3a+labs(title = "Unweighted webs", tag= "(a)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 22, face = "bold", hjust = 0.5))+
  theme(legend.position = "none")+theme(axis.title.x =element_blank())+
  fig3b+labs(title = "Weighted webs", tag="(b)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 22, face = "bold", hjust = 0.5))+
  theme(axis.title.y=element_blank())+theme(axis.text.y =element_blank())+theme(axis.title.x =element_blank())
  

jpeg(filename = "Figure 3.jpeg", width = 16, height = 8.5, units = "in", res=300)
wrap_elements(panel = fig3comb) +
  labs(tag = "Year") +
  theme(
    plot.tag = element_text(size = rel(2.5)),
    plot.tag.position = c(0.45,0.05))
dev.off()
...
THlist<-list("No threshold","90%","80%","70%","60%","50%", "40%", "30%", "20%", "10%")
df.R50list<-list()
for (i in 1:10) {
...
ggplot(uw.DF.plots, aes(sample=R50))+
  stat_qq()+
  stat_qq_line()+
  facet_wrap(~factor(threshold,levels=c("No Threshold","10%", "20%","30%","40%",
...
#### unweighted webs spearman correlation test connectance and R50 ####
cor.test (uw.DF$`No threshold`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`90%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`80%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`70%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`60%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`50%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`40%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`30%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`20%`, uw.DF$con, method=c("spearman"))
cor.test (uw.DF$`10%`, uw.DF$con, method=c("spearman"))

#### unweighted webs spearman correlation test relative ascendency and R50 ####
cor.test (uw.DF$`No threshold`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`90%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`80%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`70%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`60%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`50%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`40%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`30%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`20%`, uw.DF$`A/C`, method=c("spearman"))$estimate
cor.test (uw.DF$`10%`, uw.DF$`A/C`, method=c("spearman"))$estimate

#### weighted webs spearman correlation weighted connectance and R50####
cor.test (w.DF$`No threshold`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`90%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`80%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`70%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`60%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`50%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`40%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`30%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`20%`, w.DF$w.con, method=c("spearman"))
cor.test (w.DF$`10%`, w.DF$w.con, method=c("spearman"))

#### weighted webs spearman correlation Relative ascendency and R50####
cor.test (w.DF$`No threshold`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`90%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`80%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`70%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`60%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`50%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`40%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`30%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`20%`, w.DF$`A/C`, method=c("spearman"))
cor.test (w.DF$`10%`, w.DF$`A/C`, method=c("spearman"))
...
fig4a<-ggplot(uw.DF.plots, aes(x=con, y=R50, colour=year))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  scale_x_continuous(breaks = seq(0.15,0.27,.05),labels = scales::number_format(accuracy = 0.01),)+
  scale_color_gradient(low="yellow", high="Blue",
                       limits = c(1980, 2014), 
                       breaks = seq(1980, 2014, by = 10))+
  theme(plot.title = element_text(hjust = 0))+xlab("Connectance (C)")+ylab("R50")+
  theme(strip.text.x = element_text(size = 17))+
  theme(axis.title=element_text(size=22))+theme(legend.text=element_text(size=22),
                                                legend.title=element_text(size=22))+
  facet_wrap(~factor(threshold, levels=c("No threshold","90%", "80%","70%","60%",
                                         "50%", "40%", "30%", "20%", "10%")),ncol=5)+
  theme(panel.spacing = unit(0.68, "lines"))+
  theme(strip.background = element_blank())+theme(panel.border=element_rect(fill=NA))+
  theme(strip.text.x = element_text(size = 17))+coord_cartesian(clip = "off")+
  theme(axis.title.x = element_text(vjust = -1.7))+
...
fig4b<-ggplot(w.DF.plots, aes(x=w.con, y=R50, colour=year))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  scale_x_continuous(breaks = seq(0.05,0.16,.05),labels = scales::number_format(accuracy = 0.01))+
  scale_color_gradient(low="yellow", high="Blue",
                       limits = c(1980, 2014), 
                       breaks = seq(1980, 2014, by = 10))+
  theme(plot.title = element_text(hjust = 0))+xlab("Weighted Connectance (lwC)")+ylab("R50")+
  theme(strip.text.x = element_text(size = 17))+
  theme(axis.title=element_text(size=22))+theme(legend.text=element_text(size=22),
                                                legend.title=element_text(size=22))+
  facet_wrap(~factor(threshold, levels=c("No threshold","90%", "80%","70%","60%",
  ...
  theme(strip.text.x = element_text(size = 17))+coord_cartesian(clip = "off")+
  theme(axis.title.x = element_text(vjust = -1.7))+
  theme(plot.margin = unit(c(0,0.2,1,0.6), "cm"))

#combine into fig.4
fig4a+theme(legend.position = "none")+labs(tag = "(a)")+theme(plot.tag = element_text(size = 22,face = "bold"))+ylim(0,0.5)+
  fig4b+theme(axis.title.y=element_blank())+theme(axis.text.y=element_blank())+labs(tag = "(b)")+
  theme(plot.tag = element_text(size = 22,face = "bold"))+ylim(0,0.5)


#adding spearman rho to plots
rho<-c(cor.test (uw.DF$`No threshold`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`90%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`80%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`70%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`60%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`50%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`40%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`30%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`20%`, uw.DF$con, method=c("spearman"))$estimate,
       cor.test (uw.DF$`10%`, uw.DF$con, method=c("spearman"))$estimate)
d_text_con<-data.frame(label=c(format(round(rho,2),nsmall = 1)),
                       threshold=rev(names(table(uw.DF.plots$threshold))),
                       x=0.23,
                       y=0.05)

rho<-c(cor.test (w.DF$`No threshold`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`90%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`80%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`70%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`60%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`50%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`40%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`30%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`20%`, w.DF$w.con, method=c("spearman"))$estimate,
       cor.test (w.DF$`10%`, w.DF$w.con, method=c("spearman"))$estimate)

d_text_wcon<-data.frame(label=c(format(round(rho,2),nsmall = 1)),
                       threshold=rev(names(table(w.DF.plots$threshold))),
                       x=0.125,
                       y=0.05)
d_text_con<-d_text_con%>%mutate(label=paste("\U03C1=",label))

d_text_wcon<-d_text_wcon%>%mutate(label=paste("\U03C1=",label))


jpeg(filename = "Figure 4.jpeg", width = 16, height = 8.5, units = "in", res=300)
fig4a+labs(title = "Unweighted webs", tag= "(a)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 20, face = "bold"))+
  theme(legend.position = "none")+
  geom_text(d_text_con, mapping=aes(x=x,y=y, label=label,fontface = "italic"), size=5.5, inherit.aes=FALSE)+
  fig4b+labs(title = "Weighted webs", tag="(b)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 22, face = "bold"))+theme(axis.title.y=element_blank())+theme(axis.text.y =element_blank())+
  geom_text(d_text_wcon, mapping=aes(x=x,y=0.02, label=label, fontface = "italic"), size=5.5, inherit.aes=FALSE)
dev.off()
...
fig5a<-ggplot(uw.DF.plots, aes(x=`A/C`, y=R50, colour=year))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  scale_x_continuous(breaks = seq(0.19,0.29,.03),labels = scales::number_format(accuracy = 0.01))+
  scale_color_gradient(low="yellow", high="Blue",
                       limits = c(1980, 2014), 
                       breaks = seq(1980, 2014, by = 10))+
  theme(plot.title = element_text(hjust = 0))+xlab("Relative ascendency (A/C)")+ylab("R50")+
  theme(strip.text.x = element_text(size = 17))+
  theme(axis.title=element_text(size=22))+theme(legend.text=element_text(size=22),
                                                legend.title=element_text(size=22))+
  facet_wrap(~factor(threshold, levels=c("No threshold","90%", "80%","70%","60%",
  ...
  theme(axis.title.x = element_text(vjust = -1.7))+
  theme(axis.title.y = element_text(vjust = 1.7))+
  theme(plot.margin = unit(c(0,0.2,1,0), "cm"))
...
#adding spearman rho to plots
rho<-c(cor.test (uw.DF$`No threshold`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (uw.DF$`90%`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (uw.DF$`80%`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (uw.DF$`70%`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (uw.DF$`60%`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       ...
       cor.test (uw.DF$`20%`, uw.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (uw.DF$`10%`, uw.DF$`A/C`, method=c("spearman"))$estimate)
d_text_ac<-data.frame(label=c(format(round(rho,2), nsmall = 1)),
                       threshold=rev(names(table(uw.DF.plots$threshold))),
                       x=0.255,
                       y=0.02)

rho<-c(cor.test (w.DF$`No threshold`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`90%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`80%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`70%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`60%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`50%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`40%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`30%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`20%`, w.DF$`A/C`, method=c("spearman"))$estimate,
       cor.test (w.DF$`10%`, w.DF$`A/C`, method=c("spearman"))$estimate)

d_text_acw<-data.frame(label=c(format(round(rho,2),nsmall = 1)),
                        threshold=rev(names(table(w.DF.plots$threshold))),
                        x=0.45,
                        y=0.02)
d_text_ac<-d_text_ac%>%mutate(label=paste("\U03C1=",label))

d_text_acw<-d_text_acw%>%mutate(label=paste("\U03C1=",label))


jpeg(filename = "Figure 5.jpeg", width = 16, height = 8.5, units = "in", res=300)
fig5a+labs(title = "Unweighted webs", tag="(a)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 22, face = "bold"))+theme(legend.position = "none")+
  geom_text(d_text_ac, mapping=aes(x=x,y=y, label=label, fontface = "italic"), size=5.5, inherit.aes=FALSE)+
  fig5b+labs(title = "Weighted webs", tag="(b)")+theme(plot.title = element_text(size="20",hjust = 0.5), plot.tag = element_text(size = 22, face = "bold"))+
  theme(axis.title.y=element_blank())+theme(axis.text.y =element_blank())+
  geom_text(d_text_acw, mapping=aes(x=x,y=y, label=label, fontface = "italic"),size=5.5, inherit.aes=FALSE)
dev.off()
