#Script for SI results of the "Food web robustness depends on network type and threshold for extinction"

####READ ME####
#This file contains the script to produce the results
#seen in the SI of the article for the low to high deletion sequence
#This script assumes you have run the loop for connectnace, weighted connectance 
#and relative ascendency in the "Main_results_for_reviewers.R"
...
#The lists have a length of 1000, each entry has 34 webs inside, one for each year in the time series. So 34000 food webs total
#uw.webs.1000[[1]][[1]] is the first of 1000 un-weighted web for the year 1981, uw.webs.1000[[1]][[2]] is the first un-weighted web for 1982 and so on.
...
#NOTE: This loop is memory and time intensive. If you just want to test that it works
#you can use a smaller subset of webs and years for a greatly reduced run-time (around 2 h)
#### Unweighted webs ####
webs<-uw.webs.1000
for (i in 1:1000){ #for a smaller subset, change 1:1000 to 1:100, this limits the loop to the 100 first simulated webs.
  a<-webs[[i]]
  
  for (j in 1:34) { #for a smaller subset, change 1:34 to 1:1, this limits the loop to the webs of 1981
    fw<-a[[j]]
...
d.comb.lh
d.comb.lh$name<-factor(d.comb.lh$name, levels = c("No Threshold","90%","80%", "70%", "60%",
                                                  "50%","40%", "30%", "20%", "10%"))
...
si.fig3<-ggplot(d.comb.lh, aes(x=Year, y=value, color = name))+
  ...
  coord_cartesian(clip = "off")+theme(plot.margin = margin(0,0,2,0, "cm"))+theme(axis.title.x = element_text(vjust = -1.7))
  
  
  
png(filename = "SIFigure 3.png", width = 16, height = 8.5, units = "in", res=600)
si.fig3
dev.off()
...
si.fig5a<-ggplot(uw.DF.low_high.plots, aes(x=Con, y=R50, colour=year))+
  ...
  theme(strip.background = element_blank())+theme(panel.border=element_rect(fill=NA))+
  theme(strip.text.x = element_text(size = 17))+theme(axis.title.x = element_text(vjust = -1.7))+
  theme(axis.title.y = element_text(vjust = 1.7))+coord_cartesian(clip = "off")
...
#combined
si5<-si.fig5a+theme(legend.position = "none")+labs(tag = "(a)")+theme(plot.tag = element_text(size = 22,face = "bold"))+
  si.fig5b+theme(axis.title.y=element_blank())+theme(axis.text.y=element_blank())+labs(tag = "(b)")+
  theme(plot.tag = element_text(size = 22,face = "bold"))

png(filename = "SIFigure 5.png", width = 16, height = 8.5, units = "in", res=600)
si5
dev.off()
...
#combined
si6<-si.fig6a+theme(legend.position = "none")+labs(tag = "(a)")+theme(plot.tag = element_text(size = 18,face = "bold"))+
  si.fig6b+theme(axis.title.y=element_blank())+theme(axis.text.y=element_blank())+labs(tag = "(b)")+
  theme(plot.tag = element_text(size = 18,face = "bold"))


png(filename = "SIFigure 6.png", width = 16, height = 8.5, units = "in", res=600)
si6
dev.off()