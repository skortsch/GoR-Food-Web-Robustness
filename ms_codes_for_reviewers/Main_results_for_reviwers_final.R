...
for (i in 1:1000){ #for a smaller subset, change 1:1000 to 1:100, this limits the loop to the 100 first simulated webs.
  ...
  for (j in 1:34) {#for a smaller subset, change 1:34 to 1:1, this limits the loop to the webs of 1981
    ...
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                                  verbose = F, IS=0.5)
    #threshold, IS=0.6 means that a species is removed when 60% of its energy intake is left
    uw.R50.th50[j,i]<-ext$R50 
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                                  verbose = F, IS=0.6)
    #threshold, IS=0.7 means that a species is removed when 70% of its energy intake is left
    uw.R50.th40[j,i]<-ext$R50 
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                                  verbose = F, IS=0.7)
    uw.R50.th30[j,i]<-ext$R50 
    
    #80% threshold, IS=0.8 means that a species is removed when 80% of its energy intake is left
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                                  verbose = F, IS=0.8)
    uw.R50.th20[j,i]<-ext$R50
    #10%, IS=0.9 means a species is removed when 90% of of its energy intake is left
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                                  verbose = T, IS=0.9) #verbose=T here so you can see that the loop is running, it just prints a progress bar for each cycle of the loop
    uw.R50.th10[j,i]<-ext$R50 
  }
  if(i==1000) {print("####### THE END #######")} # Prints message when the whole loop is done
}
...
for (i in 1:1000){#for a smaller subset, change 1:1000 to 1:100, this limits the loop to the 100 first simulated webs.
  ...
  for (j in 1:34) {#for a smaller subset, change 1:34 to 1:1, this limits the loop to the webs of 1981
    ...
    w.R50[j,i]<-ext$R50#extracting R50 from output
    #90% threshold, IS=0.1 means that a species is removed when 10% of its energy intake is left
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                         verbose = F, IS=0.1)
    ...
    #10%, IS=0.9 means a species is removed when 90% of of its energy intake is left
    ext<-ExtinctionOrder(fw_i, Order = order, NetworkType = "Trophic",
                         verbose = T, IS=0.9) #verbose=T here so yo can see that the loop is running, it just prints a progress bar for each cycle of the loop
...
#Code to rearrange the unweighted and weighted R50 outputs in a dataframe to plot fig.3 in the manuscript
